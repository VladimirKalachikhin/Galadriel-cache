<?php
// возможно, какие-то пользовательские функции используют сессию. Сама система - нет.
// Но использование сессии вынуждает apache выполнять запросы к тайлам последовательно, потому что он сохраняет консистенстность сессии.
// Поэтому - нет, сессии не используем. Тогда тайлы получаются от источника параллельно, а не последовательно.
//session_start();	
