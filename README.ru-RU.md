...
2. Простой многопоточный загрузчик тайлов
3. Быстр. Особенно с [Leaflet](https://leafletjs.com/)

Всё. Нет никакой трансформации карт, никакой версионности.

## Использование:
_tiles.php_ - собственно cache/proxy 
...
Например, файл задания с именем _navionics_layer.9_, содержащий строки:
```
295,145
296,145
296,144
...
296,143
```
вызовет скачивание указанных шести тайлов карты _navionics_layer_ масштаба 9, а также всех тайлов большего масштаба, которые укладываются в эти тайлы, вплоть до масштаба 16, который указан в `params.php` как максимальный для загрузчика.
...
Например, такой файл задания _navionics_layer.9_:  
```
