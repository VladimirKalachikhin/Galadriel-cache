...
*/
require_once('fcommon.php'); // doBann($r)
...
if(@$argv) { 	// cli
	$uri = filter_var($argv[1],FILTER_SANITIZE_URL);
}
else {
...
if(!$path_parts['extension']) goto END;; 	// что-то не то с путями
...
if($pos === FALSE) {
	$mapAddPath = '';
	$mapSourcesName = $mapSourcesFile;
}
else {
	$mapAddPath = substr($mapSourcesFile,$pos);
	$mapSourcesName = substr($mapSourcesFile,0,$pos);
}
...
do {
	...
	if((!$http_response_header)) { 	 //echo "связи нет  ".$http_response_header[0]."<br>\n";
		doBann($mapSourcesName); 	// забаним источник
	...
	elseif(strpos($http_response_header[0],'403') !== FALSE) { 	// Forbidden
		...
		else 	doBann($mapSourcesName); 	// забаним источник 
		...
		foreach($http_response_header as $header) {
			...
			elseif((substr($header,0,4)=='HTTP') AND (strpos($header,'503') !== FALSE)) { 	// Service Unavailable
				if ($tries > $maxTry-1) { 	// ждём
					doBann($mapSourcesName); 	// напоследок забаним источник
					goto END;; 	 // бессмысленно ждать, уходим совсем
				}
			}
		}
...
if(($newimg !== FALSE) AND $bannedSources[$mapSourcesName]) { 	// снимем проблемы с источником, получили мы тайл или нет
	...
	umask($umask); 	// 	Вернём. Зачем? Но umask глобальна вообще для всех юзеров веб-сервера
	error_log("tiles.php: Попытка № $tries: $mapSourcesName unbanned!");
}
