...
if(!$res){	// нет живого сокета
	// случайная задержка до ...секунды, чтобы exec для всех тайлов выполнился не одновременно,
	// и хоть какой-нибудь запустился.
	// Иначе все SQLiteTilesServer стартуют одновременно, и все считают, что они уже запущены.
	// В результате не запускается ни один.
	// В целом - вся эта лабуда гарантирует запуск демона и отдачу картинки быстрее, чем
	// таймаут браузера на получение ресурса.
	//usleep(rand(0,1000000));	// случайная задержка. Должна быть тем больше, чем медленней машина сервера.
	for($i=0;$i<10;$i++){
		exec("$phpCLIexec SQLiteTilesServer.php $mapName > /dev/null 2>&1 &"); 	// запустим демона, exec не будет ждать завершения
		for($j=0;$j<20;$j++){	// демон открывает базу данных, что процесс. Надо ждать.
			$res = @socket_connect($socket,$sockName);	// оно может подключиться к кем-то запущенному демону
			if($res) break 2;
			usleep(200000);	// а надо ли ждать, пока стартует сервер? socket_connect сколько-то ждёт и так. Надо! иначе оно реально не успевает, и открытие карты вообще ни к чему не приводит.
		};
		usleep(rand(0,1000000));	// случайная задержка.
	};
};
if(!$res) return false;
//error_log("Connected!");
