...
Иначе - возвращает исходную картинку.
Если возвращает png, то соответственно меняет глобальные $ext и $ContentType.
$dColor - +- погрешность указанных цветов по каждому основному цвету
...
// нужные цвета есть в исходном полноцветном изображении
...
$backgroundImg = imagecreatetruecolor(255, 255);	// создаём полноцветную картинку

// Однако, в изображении уже может быть прозрачный цвет, определить наличие которого невозможно?
$linerColor = array(238, 238, 238);	// цвет для подмены существующего прозрачного. Удобный или незаметный.
$colorsPresent[] = $linerColor;	// добавим цвет подкладки в список имеющихся
// Сделаем замену прозрачного цвета на известный, на цвет подкладки.
list($r,$g,$b) = $linerColor;
$color = imagecolorallocate($backgroundImg,$r,$g,$b);	// регистрируем цвет
imagefill($backgroundImg, 0, 0, $color);	// закрашиваем картинку этим цветом
// копируем нашу картинку в созданную. Тут магия: после преобразования полноцветной
// картинки в палитровую прозрачный цвет закрасится цветом фона (но не точно этим!!!!)
...
// потому что imagecolorset можно только в палитровой картинке.
...
// получить только один ближайший. А поскольку надо все, то перебор цветов и получение
// точно указанного. Тем более, что imagecolorexact сильно быстрее imagecolorresolve.
// Поэтому для компенсации косяков imagetruecolortopalette и для преодоления того, что картинка
// может быть не в чистых цветах, будем искать не только указанные цвета и цвет подкладки,
// а эти цвета и цвета +- указанное отклонение.
	...
	for($rr=$r-$dColor;$rr<=$r+$dColor;$rr++){
		for($gg=$g-$dColor;$gg<=$g+$dColor;$gg++){
			for($bb=$b-$dColor;$bb<=$b+$dColor;$bb++){
				...
				$transparentColorS[] = imagecolorexact($gd_img,$rr,$gg,$bb);
			};
		};
	};
};
...
// После преобразования в палитровое изображение $linerColor изменился и/или размножился.
// Ну такая кривая imagetruecolortopalette
// Поэтому найдём его и близкие цвета.
$transparentColorS = array();
$dColor = 5;
list($r,$g,$b) = $linerColor;
for($rr=$r-$dColor;$rr<=$r+$dColor;$rr++){
...
// А теперь каждый цвет, кроме более прозрачных, заменяем на такой же, но с указанной прозрачностью
...
// Теперь из gd image сделаем обратно нормальную картику
ob_start();	// оно может быть вложенным
ob_clean();
imagepng($gd_img);
imagedestroy($gd_img);
